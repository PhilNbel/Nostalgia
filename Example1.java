...
public class Example1 extends RecursiveTask<List<String>> {
//class FolderProcessor
  ...
  public Example1(String path, String extension) {
    this.path = path;
  ...
  protected List<String> compute() {
    ...
    //Example1 tasks to store the subtasks that are going to process the subfolders stored in the folder
    List<Example1> tasks = new ArrayList<Example1>();

    //Get the content of the folder.
    File file = new File(path);

    File content[] = file.listFiles();
    //For each element in the folder, if there is a subfolder, create a new Example1 object
    //and execute it asynchronously using the fork() method.
    if (content != null) {

      for (int i = 0; i < content.length; i++) {
        if (content[i].isDirectory()) {
          Example1 task =  new Example1(content[i].getAbsolutePath(), extension);
          task.fork();
          tasks.add(task);
        }
    ...
    //If the list of the Example1 subtasks has more than 50 elements,
  ...
  private void addResultsFromTasks(List<String> list, List<Example1> tasks) {

    for (Example1 item : tasks) {
